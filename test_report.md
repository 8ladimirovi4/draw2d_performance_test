...
В среднем по трём кликам значения **AC_02** по фикстурам близки (см. таблицу выше).
...
| Источник                                                                                     | Скриншот small                                                 | Скриншот big                                               |
| ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ | -------------------------------------------------------------------- |
| Список heap snapshots и Summary в Memory (все этапы 1–5 на одном кадре) | [poc1-req02-small_memory.png](images/AC_3/poc1-req02-small_memory.png) | [poc1-req01-big_memory.png](images/AC_3/poc1-req01-big_memory.png) |
...
Ниже — **качественный** разбор записей **Performance** для **обеих** фикстур: дорожка **Frames** (длительность кадра, жёлтые кадры), **INP** в Insights и **Main** (длинные **`mousemove`** / **`onMouseDrag`**).
...
| **Дорожка Frames**           | У каждого кадра подписана**длительность** (мс). Для **60 Hz** ориентир одного кадра **~16.7 ms**; для **30 FPS** — **~33.3 ms**. **Зелёный** кадр — укладывается в бюджет; **жёлтый** — кадр с проблемами по эвристике DevTools (часто при тяжёлой работе на Main при подписи всё ещё около **16.7 ms**). На скриншотах в **`images/AC_4/`** проблемные кадры **жёлтые**, без красной полоски сверху. |
...
| **Frames**   | На**`poc1-req01-big_frames_1.png`** после начала переноса (порядка **~1.6–4.8 s** взаимодействия на записи **~5.3 s**) дорожка **Frames** — сплошные **жёлтые** кадры; подписи длительности часто **~16.5–16.7 ms** при жёлтом цвете (см. обсуждение эвристик DevTools в переписке). На **`poc1-req01-big_frames_2.png`** — детализация: крупные **`Event: mousemove`**, плотный стек вызовов при drag, завершение с **`mouseup`** / **`commitTransaction`**. |
| **Main**     | Те же узкие места: синхронная обработка**каждого** движения мыши по **всему** объёму большой схемы — **long tasks**, красная полоса long tasks в обзоре CPU на отрезке drag.                                                                                                                                                                                                                                                                                                                                                                               |
